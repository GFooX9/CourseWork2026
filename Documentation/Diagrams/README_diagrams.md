...
![Диаграмма деятельности](./Activity_diagram.png)

Диаграмма деятельности описывает действия, которые выполняет пользователь в процессе инициализации и верификации результатов ин-теллектуального анализа изображений в системе, включая этапы нейросетевого инференса моделей YOLO и OCR, логические проверки и переходы между состояниями, а также синхронное взаимодействие мо-дулей приложения с файловым кэшем. Пользователь выбирает изобра-жение, а UI загружает его в кэш, запускает потоки, блокирует элементы окна и выводит прогресс-бар. Параллельно в Engine запускается Vision Engine, где по условию выполняется либо последовательный инференс YOLO и OCR, либо только OCR. После этого результаты агрегируются, и сигнал передается в главный поток. В завершение UI скрывает про-гресс-бар, разблокирует интерфейс и выдает итоговое изображение.

# Описание диаграммы Последовательности

![Диаграмма последовательности](./Sequence_diagram.png)

Процесс отображения архива исследований начинается с того, что пользователь открывает экран истории исследований HistoryScreen в интерфейсе приложения. После этого интерфейсная часть автоматически формирует внутренний запрос к базе данных для извлечения всех ранее сохраненных записей. Получив запрос, локальная СУБД SQLite обра-щается к таблице history и извлекает накопленные метаданные. База данных выполняет чтение и возвращает интерфейсу массив записей, по-сле чего интерфейс динамически генерирует на экране графические кар-точки-протоколы и уведомляет пользователя об успешном выводе архи-ва.


Дополнительно в диаграмме отражен необязательный процесс (блок opt) оперативной фильтрации данных по временным периодам («Живой поиск»). В случае необходимости пользователь вводит поиско-вый запрос (номер протокола) или выбирает временной интервал («Се-годня», «За неделю»). Интерфейсная часть инициирует отдельный филь-трующий запрос к базе данных. Компонент DatabaseManager выполня-ет текстовый поиск по оператору LIKE средствами SQL, после чего осу-ществляет кадрирование временных периодов путем выгрузки данных в структуру DataFrame библиотеки pandas и возвращает в графический интерфейс оптимизированный массив словарей, получив статус успеш-ного завершения поиска, мгновенно обновляет отображаемый список на экране пользователя.
